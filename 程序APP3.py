            ...
            try:
                ...
                try:
                    ...
                    if isinstance(shap_values, list) and len(shap_values) > 1:
                        # 二分类问题，使用第二个类别（通常是正类/死亡类）
                        feature_shap_values = shap_values[1][0]
                    ...
                    # 按照绝对值大小排序
                    impact_df['绝对值'] = np.abs(impact_df['SHAP值'])
                    impact_df = impact_df.sort_values('绝对值', ascending=False)
                    
                    # 设置颜色（红色增加风险，绿色降低风险）
                    impact_df['颜色'] = ['red' if x > 0 else 'green' for x in impact_df['SHAP值']]
                    impact_df['方向'] = ['增加风险' if x > 0 else '降低风险' for x in impact_df['SHAP值']]
                    
                    # 创建横向条形图
                    fig, ax = plt.subplots(figsize=(10, 6))
                    bars = ax.barh(impact_df['特征'], impact_df['SHAP值'], color=impact_df['颜色'])
                    ...
                    # 设置图表标题和标签
                    ax.set_title('特征对预测的影响', fontsize=14)
                    ax.set_xlabel('SHAP值 (红色=增加风险, 绿色=降低风险)', fontsize=11)
...
# 修改页脚，缩小边距
st.markdown("""
<div class="disclaimer" style="margin-top: 0.5rem; padding-top: 0.5rem;">
